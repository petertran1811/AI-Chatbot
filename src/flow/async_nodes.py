...
from flow.state import State, get_messages, get_revert_messages
from flow.prompts import get_prompt


async def a_call_model(state: StopAsyncIteration, config: RunnableConfig):
...
async def a_summarize_conversation(state: State):
    summary = state.get("summary", "")
    if summary:
...
async def a_suggestion_for_user(state: State):
    """
    ...
    response = await MODEL_STRUCTURED_OUTPUT.ainvoke(
        input=[
